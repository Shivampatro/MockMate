...
import { doc, getDoc, collection, query, where, limit, getDocs, orderBy, setDoc } from "firebase/firestore";
...
export async function getInterviewById(id: string): Promise<Interview | null> {
  ...
  return interviewSnap.data() as Interview | null;
}
...
export async function getLatestInterviews(
  ...
  const { userId, limit: limitCount = 20 } = params;
  ...
  const interviewsSnap = await getDocs(q);
